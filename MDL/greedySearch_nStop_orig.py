...
#    print mContent[0];
    ...
    while times <= maxStructs : #min(structLim, maxStructs) :  # add upto structLim structures or as many as there are in the model file  
       ...
       if l_total_x > l_total_prev :
          ...
          l_error_x = l_error_prev;
	  E_x = E_x_old;
          if config.optDebug:
		print "new modelling errors check: " + str(E_x.numModellingErrors);
		print "old modelling errors check: " + str(E_x_old.numModellingErrors);
       ...
       times += 1;
    #output final cost
    target = open('out_final.out','w');
    target.write("M_x:\t" + '%.0f' % l_total_prev + "\t" + '%.0f' % l_model_x + "\t" + '%.0f' % l_error_prev + "\t" + str(E_x_old.numModellingErrors) + '/' + str(E_x_old.numCellsCovered) + '\t' + str(E_x_old.numUnmodelledErrors)  + '/' + str(((E_x_old.numNodes * E_x_old.numNodes)-E_x_old.numNodes) - E_x_old.numCellsCovered) + '\t\t' + str(E_x_old.numCellsExcluded));
    target.close();
